...
- **Firebase sync** (`FirebaseSyncService.syncAll`, launch when signed in): uses the **exact same Firestore schema as the iOS app** so the two clients share data (see "Firestore schema" below). Single-item uploads fire from repo mutation hooks (wired in `App.kt`). `lastSyncDate` is stamped in `finally` (always), and `lastSyncError` surfaces failures (Account "Sync Now" shows a Toast).
...
## Firestore schema (MUST stay identical to iOS — the two apps share the same data)
Docs live under `users/{uid}/<collection>`. Android finds/updates docs with a `whereEqualTo(<key>)` query (matching iOS's `.addDocument` + `whereField`) so there's **one doc per key across both platforms** — do NOT switch to deterministic/Base64 doc ids (that creates duplicates vs iOS). Dates are Firestore **`Timestamp`s** (Room stores millis → convert at the boundary with `Timestamp(Date(millis))` / `doc.getTimestamp(..)?.toDate()?.time`).

| Collection | Key field | Other fields | Strategy |
|---|---|---|---|
| `favorites` | `linkToDetails` | name, thumbnail, linkToWatch, watchMovieLinkWithEpisodeId, type (`"Movie"`/`"TV Show"`), description, quality, cast, genre, duration, country, imdb, release, production, `addedAt`(Ts), `seasonsList`(array) | presence (upload new / download missing / delete orphaned) |
| `watched` | `linkToDetails` | `watchedAt`(Ts); Android also writes name/thumbnail/type (iOS ignores) | presence + delete |
| `watchedEpisodes` | `episodeLink` | `watchedAt`(Ts); Android also writes showLink/seasonNumber/episodeNumber (iOS ignores) | presence |
| `playbackProgress` | `contentLink` | **`position`/`duration` in SECONDS (Double)**, `updatedAt`(Ts), title, thumbnail, contentType | last-writer-wins by `updatedAt` |
| `episodeSnapshots` | `linkToDetails` | `knownEpisodeLinks`(String array), `lastCheckedAt`(Ts) | last-writer-wins |
| `showNotifications` | `showLinkToDetails` | showName, showThumbnail, newEpisodeCount, latestEpisodeNumber, latestEpisodeName, `detectedAt`(Ts), isRead | last-writer-wins by `detectedAt` + isRead propagation |

**Key iOS-compat rules (violate these and cross-device breaks silently):**
- Playback `position`/`duration` are **seconds**, not `positionMs`. Room ↔ cloud converts `/1000.0` and `*1000`.
- iOS stores **only `episodeLink` + `watchedAt`** for a watched episode — **no showLink**. So on Android, decide "is this episode watched" by the **global** set of watched episode links (`WatchedEpisodeRepository.allWatchedLinks()`), never `watchedLinksForShow(...)`. Used by details watched-checks, the current-watching-season pick, and `ContinueWatchingRepository`.
- The **Watched screen = favorites ∩ watched links** (`WatchedViewModel`), pulling name/thumbnail from the favorite — the `watched` docs only carry `linkToDetails`, so displaying `WatchedMovie` rows directly renders blank for synced items.
- Mark-as-read: `NotificationsViewModel.open()` posts the list (unread dots visible), then `markAllRead()` + `FirebaseSyncService.markAllNotificationsRead()` (sets `isRead=true` on every cloud doc), then re-posts so the dots clear.
...
- **D-pad focus selection belongs to TV only, not tablets.** The list adapters (AllMovies/AllTvShows/Favorites/Genre/Search) used `isTabletOrientation()` to enable `requestFocus()`/focus-selection — on a **touch tablet** that caused random cards to get selected (red border) while scrolling. Use **`Context.isTvDevice()`** (leanback feature / live-tv / UI-mode television) — defined in `MainActivity.kt` — for anything that should only happen on a real TV.
- **Google Sign-In needs the signing SHA-1 registered in Firebase.** "Pick an account, nothing happens" = `ApiException` status **10 (DEVELOPER_ERROR)**; `google-services.json` must contain an **Android OAuth client (`client_type` 1)** for the package + SHA-1 (it may only have the web client, type 3). `AccountFragment` now Toasts the status code. Debug SHA-1: `keytool -list -v -keystore ~/.android/debug.keystore -alias androiddebugkey -storepass android`. Release/Play builds need their own SHA-1s too.
- **Pagination must accumulate.** `AllMovies/AllTvShows/MovieByGenre` view models originally **replaced** the list with each new page (only the Filter screen appended). Fixed to `list = (list + newPage).distinctBy { it.link }`. The list adapters' `setMovies` replaces, so the VM must hold the full accumulated list.
- **Account** screen is a full iOS-style redesign (person-circle avatar, name + email, white "Sign in with Google", red "Sign Out"); **splash** uses the real iOS `SplashLogo` (`res/drawable-nodpi/splash_logo.png`) centered on black; **tablet movie details** is a side-by-side layout in `res/layout-sw600dp/` driven by `details_*` dimens in `values-sw600dp`/`values-sw720dp`; player button sizes scale via `player_*` dimens. Watched-episode indicator is a **green circle + white check** (`ic_check_circle_green`).
